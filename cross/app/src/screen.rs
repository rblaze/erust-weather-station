...
impl<I2cBus: I2c> Lcd<I2cBus> {
    ...
    pub fn set_output_line(&mut self, line: usize) -> Result<(), Error<I2cBus::Error>> {
        if line > 1 {
...
impl<I2cBus: I2c> Screen<WIDTH, HEIGHT, I2cError<I2cBus::Error>> for Lcd<I2cBus> {
    fn send_command(&mut self, command: u8) -> Result<(), I2cError<I2cBus::Error>> {
        self.i2c
            .write(I2C_ADDR, &[TransactionMode::Command as u8, command])?;
        Ok(())
    }

    fn send_data(&mut self, data: u8) -> Result<(), I2cError<I2cBus::Error>> {
        self.i2c
            .write(I2C_ADDR, &[TransactionMode::Data as u8, data])?;
        Ok(())
    }

    fn send_data_bytes(&mut self, data: &[u8]) -> Result<(), I2cError<I2cBus::Error>> {
...
impl<I2cBus: I2c> core::fmt::Write for Lcd<I2cBus> {
