...
impl I2c<I2C1> {
    ...
    fn wait_for_txis(&self, transfer_kind: i2c::NoAcknowledgeSource) -> Result<(), Error> {
        loop {
    ...
    fn send_buf(&self, wrbuf: &[u8]) -> Result<(), Error> {
            ...
            debug_assert!(
                {
                    let status = self.i2c.isr.read();
                    !wrbuf.is_empty()
                        || (status.tc().is_not_complete() && status.tcr().is_not_complete())
                },
                "unexpected I2C write completion, isr: {:016b}",
                self.i2c.isr.read().bits(),
            );

            self.wait_for_txis(source)?;
        ...
        debug_assert!(
            {
                let status = self.i2c.isr.read();
                status.tc().is_complete() || status.tcr().is_complete()
            },
            "unexpected I2C write non-completion, isr: {:016b}",
            self.i2c.isr.read().bits(),
        );
    ...
    fn read_buf(&self, rdbuf: &mut [u8]) -> Result<(), Error> {
        ...
        for byte in rdbuf {
            debug_assert!(
                ...
                "unexpected I2C read completion, isr: {:016b}",
                self.i2c.isr.read().bits(),
            );
        ...
        debug_assert!(
            ...
            "unexpected I2C read non-completion, isr: {:016b}",
            self.i2c.isr.read().bits(),
        );
    ...
    fn transaction(
        ...
        match operations.last_mut() {
            ...
            Some(i2c::Operation::Write(wrbuf)) => {
                ...
                self.send_buf(wrbuf)?;
                debug_assert!(self.i2c.isr.read().tc().is_complete());
            }
